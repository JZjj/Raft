...
func (rf *Raft) readPersist(data []byte) {
	...
	if d.Decode(&currentTerm) != nil ||
		...
		rf.log = log
		// Ensure the dummy entry at index 0 is always present
		if len(rf.log) == 0 || rf.log[0].Term != 0 {
			rf.log = append([]LogEntry{{Term: 0, Command: nil}}, rf.log...)
		}
	}
}
...
func (rf *Raft) leaderHeartBeat() {
	...
	for {
		...
		for server := range rf.peers {
			...
			go func(server int) {
				// For heartbeats, use the follower's current log length
				rf.mu.Lock()
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.log = make([]LogEntry, 1)                // Start with index 1, so log[0] is a dummy entry
	rf.log[0] = LogEntry{Term: 0, Command: nil} // Dummy entry at index 0
	rf.timestamp = time.Now()
	rf.state = FOLLOWER
	rf.votedFor = -1
